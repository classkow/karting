...
export function buildChassis(root, reg) {
  ...
  const backLo = new THREE.Mesh(new RoundedBoxGeometry(0.21, 0.20, 0.02, 3, 0.01), M.fabric);
  backLo.position.set(0, 0.375, -0.082);
  backLo.rotation.x = -0.369; // 腰背段后仰,前面贴住内表面
  const backHi = new THREE.Mesh(new RoundedBoxGeometry(0.21, 0.12, 0.02, 3, 0.01), M.fabric);
  backHi.position.set(0, 0.505, -0.123);
  backHi.rotation.x = -0.226; // 肩靠段趋直,前面贴住内表面;两段 y 向重叠,不留壳体缝
